...
public class entry_2525454 {
    ...
    static void solve() throws IOException {
        ...
        for(int i = 0; i < n; i++) {
            projects.add(new project(sc.nextInt(), sc.nextInt(), sc.nextInt()));
        }
 
        Comparator<project> cmp = new Comparator<>() {
            @Override
        ...
        for(int i = 0; i < n; i++) {
            ...
            Integer key = tm.lowerKey(p.st);
            long temp = tm.get(key) + p.val;
            ans = Math.max(ans, temp);
            if(!tm.containsKey(p.end)) {
                int lowerKey = tm.lowerKey(p.end);
                tm.put(p.end, tm.get(lowerKey));
            }
            tm.replace(p.end, Math.max(tm.get(p.end), temp));
        }
    ...
    static class project {
        ...
        public project(int st, int end, int val) {
            this.st = st;
            this.end = end;
            this.val = val;
        }
