# 1 "Sorting-and-Searching/Distinct-Numbers.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 383 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "Sorting-and-Searching/Distinct-Numbers.cpp" 2
# 1 "Sorting-and-Searching/../lib.cpp" 1
...
# 39 "/usr/bin/../lib/gcc/x86_64-linux-gnu/9/../../../../include/c++/9/cstdint" 2 3
...
# 2 "Sorting-and-Searching/../lib.cpp" 2
...
# 3 "Sorting-and-Searching/../lib.cpp" 2
# 1 "/usr/bin/../lib/gcc/x86_64-linux-gnu/9/../../../../include/c++/9/cstdarg" 1 3
# 40 "/usr/bin/../lib/gcc/x86_64-linux-gnu/9/../../../../include/c++/9/cstdarg" 3
# 53 "/usr/bin/../lib/gcc/x86_64-linux-gnu/9/../../../../include/c++/9/cstdarg" 3
namespace std
{
  using ::va_list;
}
# 4 "Sorting-and-Searching/../lib.cpp" 2
...
typedef void Void;
typedef Char *String;
# 45 "Sorting-and-Searching/../lib.cpp"
template <typename T>
Void swap(T *x, T *y)
{
    T tmp = *x;
...
Void memory_fill(Void *memory, Int length, UInt8 value)
{
...
{
    UInt8 *dest_data = (UInt8 *)dest;
    UInt8 *src_data = (UInt8 *)src;
    while (length--)
    {
        *dest_data++ = *src_data++;
    }
...
Bool memory_equals(Void *mem, Void *mem2, Int length)
{
    UInt8 *mem_data = (UInt8 *)mem;
    UInt8 *mem2_data = (UInt8 *)mem2;
    while (length--)
    {
        if (*mem_data++ != *mem2_data++)
        {
            return (0);
        }
    }
...
Void read_line(String line, Int *length)
{
    fgets(line, 0x7fffffff, stdin);
...
Void write_format(String format, ...)
{
    va_list args;
...
Float binary_exp(Float a, Int b)
{
    Float res = 1;
    while (b > 0)
    {
...
template <typename T>
T *matrix_ptr(T *mat, Int n, Int i, Int j)
{
    return mat + i * n + j;
}
 
template <Int M>
Void matrix_mul(Int *mat1, Int *mat2, Int *mat3, Int n)
{
    for (Int i = 0; i < n; i++)
    {
        for (Int j = 0; j < n; j++)
        {
            ...
            for (Int k = 0; k < n; k++)
            {
                value = (value + *matrix_ptr(mat1, n, i, k) * *matrix_ptr(mat2, n, k, j) % M) % M;
            }
            *matrix_ptr(mat3, n, i, j) = value;
        }
    }
}
 
Void matrix_mul(Float *mat1, Float *mat2, Float *mat3, Int n)
{
    for (Int i = 0; i < n; i++)
    {
        for (Int j = 0; j < n; j++)
        {
            ...
            for (Int k = 0; k < n; k++)
            {
                value = value + *matrix_ptr(mat1, n, i, k) * *matrix_ptr(mat2, n, k, j);
            }
            *matrix_ptr(mat3, n, i, j) = value;
        }
    }
...
template <Int M>
Int *matrix_exp(Int *mat1, Int *mat2, Int *mat3, Int n, Int e)
{
    for (Int i = 0; i < n; i++)
    {
        for (Int j = 0; j < n; j++)
        {
            *matrix_ptr(mat2, n, i, j) = i == j ? 1 : 0;
        }
    }
    while (e > 0)
    {
        if (e & 1)
        {
            matrix_mul<M>(mat1, mat2, mat3, n);
            swap(&mat2, &mat3);
        }
        matrix_mul<M>(mat1, mat1, mat3, n);
        swap(&mat1, &mat3);
        e >>= 1;
    }
    return mat2;
...
Float *matrix_exp(Float *mat1, Float *mat2, Float *mat3, Int n, Int e)
{
    for (Int i = 0; i < n; i++)
    {
        for (Int j = 0; j < n; j++)
    ...
    while (e > 0)
    {
        if (e & 1)
        {
            matrix_mul(mat1, mat2, mat3, n);
            swap(&mat2, &mat3);
        }
        matrix_mul(mat1, mat1, mat3, n);
        swap(&mat1, &mat3);
        e >>= 1;
    }
    return mat2;
...
template <typename T>
Void count_item(Int *count, T *items, Int n, T offset)
{
    for (Int i = 0; i < n; i++)
    {
        count[items[i] - offset]++;
    }
}
...
template <Int M>
Void sub_factorial(Int *fact, Int n)
{
    fact[0] = 1;
    fact[1] = 0;
    for (Int i = 2; i <= n; i++)
    {
        fact[i] = (i - 1) * ((fact[i - 1] + fact[i - 2]) % M) % M;
    }
}
...
Int fibnacci(Int n)
{
    ...
    Int mat1[2 * 2] = {0, 1, 1, 1};
    Int mat2[2 * 2];
    Int mat3[2 * 2];
    Int *mat = matrix_exp<M>(mat1, mat2, mat3, 2, n - 1);
    return *matrix_ptr(mat, 2, 1, 1);
}
 
Void sieve_prime(Int *factor, Int n, Int *prime, Int *pn_res)
{
    Int pn = 0;
    for (Int i = 2; i <= n; i++) {
        if (factor[i] == 0) {
        ...
        for (Int j = 0; j < pn && prime[j] <= factor[i] && i * prime[j] <= n; j++)
        {
...
Int mod_inv(Int x)
{
    return binary_exp<M>(x, M - 2);
}
# 2 "Sorting-and-Searching/Distinct-Numbers.cpp" 2
