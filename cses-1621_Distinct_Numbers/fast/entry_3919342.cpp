...
static constexpr int MAX = 1'000'000'000;
static constexpr int SHIFT = 23;
static constexpr int MASK = (1 << SHIFT) - 1;
static constexpr int BLOCKS = 1 + ((MAX - 1) >> SHIFT);
 
std::bitset<1 << SHIFT> b;
std::vector<unsigned> chunks[BLOCKS];
 
int main() {
    for (int i = 0; i < BLOCKS; ++i) chunks[i].reserve(1000);
    unsigned n;
    cin >> n;
    while (n--) {
        unsigned x;
        cin >> x;
        chunks[x >> SHIFT].push_back(x & MASK);
    }
    int ans = 0;
    for (int i = 0; i < BLOCKS; ++i) {
        for (auto x : chunks[i])
            if (!b[x]) b[x] = true, ++ans;
        for (auto x : chunks[i]) b[x] = false;
    }
    cout << ans << '\n';
}