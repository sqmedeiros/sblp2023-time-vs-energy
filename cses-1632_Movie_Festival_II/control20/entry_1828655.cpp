...
int main() {
	...
	for (int i = 0; i < n; i++) {
		auto it = end_times.upper_bound(v[i].second); //check for time above starting time
		...
		end_times.insert(v[i].first); // insert end time
		++ ans;
 
		// the idea is that if there is a end time <= start time of next it will not be the beginning of the set
	}
