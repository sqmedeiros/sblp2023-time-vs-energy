...
int main() {
    ...
    int cnt = 0;
    for (auto &[val, _v] : comp) {
        _v = cnt++;
    }
 
    seg_tree tree(cnt + 1);
    tree.build(vector<segment>(cnt, segment(0, 0, 0, 0, 0)));
    int ans = 0;
    for (auto &[end, start] : mov) {
        int b = comp[end], a = comp[start];
        segment seg = a == 0 && b == n ? tree.query_full() : tree.query(a, b);
        if (seg.maximum < k) {
            tree.update(a, b, segment_change(int(1)));
            ans++;
        }
    }
    cout << ans << '\n';
